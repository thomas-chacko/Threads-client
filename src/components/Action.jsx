...
const Action = ({ post: post_ }) => {
  ...
  const handleLikeAndUnlike = async () => {
    if (!user) {
      showTost("Error", "You need to login to like posts", "error");
      return;
    }
    if (liking) return;
    setLiking(true);
  ...
  const handleReply = async () => {
    if (!user) {
      showTost("Error", "You need to login to reply posts", "error");
      return;
    }
    try {
      ...
      showTost("Success", "Reply added successfully", "success");
      onClose();
      setReply("");
    } catch (error) {
      showTost("Error", error, "error");
    ...
    <Flex flexDirection={"column"}>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button onClick={handleReply} colorScheme="blue" mr={3} size={"sm"}>
              Post
            </Button>
